...
func (p PairList) Swap(i, j int)      { p[i], p[j] = p[j], p[i] }        //交换
func (p PairList) Len() int           { return len(p) }                  //查询长度
...
/*
   基于map实现了类型WordCount, 并对期实现了Merge(), Report(), SortReport(), UpdateFreq(),WordFreqCounter() 方法
*/
